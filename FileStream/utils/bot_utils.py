...
async def is_user_joined(bot, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            text=(
                "<i>Sᴏᴍᴇᴛʜɪɴɢ ᴡʀᴏɴɢ "
                "ᴄᴏɴᴛᴀᴄᴛ ᴍʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ</i>"
                f" <b><a href='https://t.me/"
                f"{Telegram.UPDATES_CHANNEL}'>"
                "[ ᴄʟɪᴄᴋ ʜᴇʀᴇ ]</a></b>"
            ),
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True,
...
def get_secure_base_url():
    url = getattr(
        Server,
        "URL",
        "https://filestream-isn7.onrender.com/",
    )
...
async def is_user_exist(bot, message):
    ...
    # Logging must NEVER prevent the bot from processing the file.
    try:
...
async def is_channel_exist(bot, message):
    if await db.get_user(message.chat.id):
        return

    await db.add_user(message.chat.id)

    try:
        members = await bot.get_chat_members_count(
            message.chat.id
        )
